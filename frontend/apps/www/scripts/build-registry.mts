...
`
  ...
  for (const item of registry) {
    const resolveFiles = item.files?.map(
      (file) =>
        `registry/default/${typeof file === "string" ? file : file.path}`
    )
    if (!resolveFiles) {
      continue
    }
    ...
    if (item.type === "registry:block") {
      ...
      if (item.files) {
        ...
        if (files?.length) {
          sourceFilename = `__registry__/default/${files[0].path}`
        }
      }
    ...
    let componentPath = `@/registry/default/${type}/${item.name}`
    ...
    if (item.files) {
      ...
      if (files?.length) {
        componentPath = `@/registry/default/${files[0].path}`
      }
    }
